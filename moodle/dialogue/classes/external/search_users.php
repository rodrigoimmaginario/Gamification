...
class search_users extends external_api {
    ...
    public static function execute(int $cmid, string $search, bool $searchanywhere, int $page, int $perpage): array {
        ...
        // Perfis que são aceitos
        $allow_rules = explode(',', $pluginconfig->roles);
        ...
        foreach ($users['users'] as $user) {
            ...
            // Adiciona somente os usuários com o perfil permitido
            $roles = get_user_roles($context, $user->id, true);
            $rk = array_keys($roles);

            if (!in_array($roles[$rk[0]]->roleid, $allow_rules)) {
                continue;
            }
