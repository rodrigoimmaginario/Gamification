...
class conversations_by_author extends conversations {
    ...
    public function setup() {
        ...
        if (!has_capability('mod/dialogue:viewany', $this->dialogue->context)) {
            ...
            $this->basesql .= " JOIN (SELECT dp.conversationid FROM {dialogue_participants} dp
    ...
    public function set_filter($filter) {
        $this->filter = ' AND dc.subject LIKE "%' . str_replace(' ', '%', $filter) . '%" ';
    }
     ...
     * @throws \moodle_exception
     */
    public function records_filter() {
        global $DB;
        ...
        $select = "SELECT $fields $this->basesql $this->filter $this->orderbysql";
