...
function dialogue_get_user_details(\mod_dialogue\dialogue $dialogue, $userid) {
    ...
    if (!$cache->get($context->id)) {
        ...
        foreach ($enrolledusers as &$enrolleduser) {
            ...
            dialogue_add_user_group_fields($enrolleduser, $dialogue->course->id);
        }
    ...
    if (!isset($cachedusers[$userid])) {
        ...
        dialogue_add_user_group_fields($user, $dialogue->course->id);
 ...
 * @param stdClass $user
 */
function dialogue_add_user_group_fields(stdClass &$user, string $courseid)
{
    global $DB;
    ...
    try {
        $result = $DB->get_records_sql($sql, [$user->id, $courseid]);
    } catch (dml_exception $e) {
        $user->group = 'Este usuário não pertence a nenhum grupo';
        return;
    }
    ...
    if (0 == $ttl) {
        ...
        $user->group = substr($user->group, 0, -2);
    }
    return;
}
 ...
 * @return string html
 */
function dialogue_generate_summary_line($subject, $body, $bodyformat, $length = 120, $separator = ' - ') {
    ...
    return html_writer::tag('strong', $subject) .
...
/**
 * Seleciona os perfis disponíveis no Moodle
 *
 * @return array
 * @throws dml_exception
 */
function dialogue_get_rules()
{
