...
if ( ('' !== $filter) || (!$filter)) { $pageurl->param('filter', $filter); } /**/
...
$PAGE->requires->js(new moodle_url($CFG->wwwroot . '/mod/dialogue/js/search-filter.js')); 
...
echo $renderer->search_form($filter);
...
echo $renderer->conversation_listing_filter($list);
