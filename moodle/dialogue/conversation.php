...
// Adiciona o script se usuário não for estudante
$user_roles = get_user_roles($context, $USER->id);
$role = '';
            
foreach ($user_roles as $roles) {
    $role = $roles->shortname;
}

if ($role != 'student') {    
    $PAGE->requires->js(new moodle_url($CFG->wwwroot . '/mod/dialogue/js/select-user.js'));    
    $PAGE->requires->js(new moodle_url($CFG->wwwroot . '/mod/dialogue/js/disable-buttons.js'));    
} else {
    $PAGE->requires->js(new moodle_url($CFG->wwwroot . '/mod/dialogue/js/disable-buttons.js'));    
}
