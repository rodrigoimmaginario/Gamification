...
class mod_dialogue_renderer extends plugin_renderer_base {
    ...
    public function render_conversation(mod_dialogue\conversation $conversation) {
        ...
        $html .= html_writer::start_div('conversation-actions');
        ...
        if ($conversation->state == \mod_dialogue\dialogue::STATE_OPEN) {
            ...
            $html .= html_writer::tag('div', $span, array('class' => 'heading'));
        }
        ...
        $groupauthor = $conversation->author->group;
        $html .= '</br>';
        $html .= html_writer::tag('span', $groupauthor );        
    ...
    public function conversation_listing(\mod_dialogue\conversations $conversations) {
        ...
        if (empty($list)) {
            ...
            foreach ($list as $record) {
                ...
                $html .= html_writer::tag('td', $statelabel, array('class' => 'dialogue_status-list'));
                ...
                if (isset($record->unread)) {
                    ...
                    $html .= html_writer::tag('td', $badge, array('class' => 'dialogue_badge-list'));
                }
                ...
                if (isset($record->userid)) {
                    ...
                    // Mostrando nome e grupo
                    $display = fullname($displayuser);
                    $display .= ($displayuser->group != '') ? '<span>' . $displayuser->group . '</span>' : '';


                    $html .= html_writer::tag('td', $display, array('class' => 'dialogue_avatar-list'));
                }
                ...
                if (isset($record->subject) and isset($record->body)) {
                    ...
                    $html .= html_writer::start_tag('td', array('class' => 'dialogue_content-list'));
                ...
                if (isset($record->timemodified)) {
                    ...
                    $html .= html_writer::tag('td', $datetime, array('title' => userdate($record->timemodified), 'class' => 'dialogue_date-list'));
                }
    ...
    public function tab_navigation(\mod_dialogue\dialogue $dialogue) {
        ...
        // Search
        /*$active = ($currentpage == 'search') ? array('class' => 'active') : array();
        $html .= html_writer::start_tag('li', $active);
        $draftsurl = new moodle_url('search.php', array('id' => $cm->id));
        $html .= html_writer::link($draftsurl, get_string('search', 'dialogue'));
        $html .= html_writer::end_tag('li');*/


        $html .= html_writer::end_tag('ul');

        $html .= html_writer::start_div('caps-content');
        $html .= html_writer::start_div('caps-button');
            ...
            if (has_capability('mod/dialogue:open', $context)) {
                ...
                $html .= html_writer::link($createurl, 'Nova conversa', array('class' => 'btn-create btn-primary btn-modify-cead'));
            }
        $html .= html_writer::end_div();
        return $html;
    }

    public function search_form($filter = '') {
        $html = '';

        $html .= html_writer::start_div('div-pesquisar');
        ...
        $html .= html_writer::start_tag('input', array('type' => 'text', 'name' => 'filter', 'value' => $filter, 'placeholder' => 'Pesquisar...', 'style' => 'margin-bottom: 10px;'));
        $html .= html_writer::end_div();
        $html .= html_writer::start_div('col-md-auto');
        $html .= html_writer::start_tag('input', array('type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('search')));   
        $html .= '&nbsp;&nbsp;';     
        $html .= html_writer::start_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary' , 'value' => get_string('clear'), 'id' => 'btn-clean-filter'));
        $html .= html_writer::end_div();        
        $html .= html_writer::end_div();
        $html .= html_writer::end_div();
     ...
     * @return string
     */
    public function conversation_listing_filter(\mod_dialogue\conversations $conversations) {
        global $DB;
        $dialogue = $conversations->dialogue;
        $cm       = $conversations->dialogue->cm;
        ...
        if ($rowsmatched) {
            $list = $conversations->records_filter();
        }
        ...
        if (empty($list)) {
            ...
            foreach ($list as $record) {
                ...
                if (isset($record->userid)) {                          
                    ...
                    $courseid = $cm->course;  
                    //consulta para retornar id do usuário que iniciou a conversa
                    $sql = "SELECT msg.authorid AS primary_author FROM {dialogue_messages} AS msg
                            INNER JOIN {dialogue_conversations} AS cv ON
                            cv.id = msg.conversationid
                            INNER JOIN {dialogue} AS dlg ON
                            dlg.id = cv.dialogueid
                            WHERE cv.course = ".$courseid."
                            AND msg.conversationid = ".$conversationid."
                            AND msg.conversationindex IN (SELECT MIN(conversationindex) FROM {dialogue_messages} WHERE conversationid = ".$conversationid.")";
                    $responsesql = $DB->get_record_sql($sql);                    
                    $newdisplay = dialogue_get_user_details($dialogue, $responsesql->primary_author); 
                    $html .= html_writer::start_tag('td', array('class' => 'dialogue_content-list'));
                    $html .= '<span>' .$newdisplay->group. '</span>';
                    $html .= html_writer::end_tag('td');
    ...
    public function state_button_group() {
        ...
        $html .= html_writer::start_div('caps-right');
    ...
    public function list_sortby($options, $sort, $direction) {
        ...
        $html .= html_writer::start_div('js-control btn-group');

        $html .= html_writer::start_tag('button', array('data-toggle' => 'dropdown',
                                                        'class' => 'btn btn-secondary margin-spec btn-small dropdown-toggle'));
