...
function get_user_from_course($context)
{
    global $DB;
    ...
    $sql = 'SELECT course FROM {course_modules} WHERE id = ?';
    $courseid = $DB->get_record_sql($sql, [$context]);

    // Seleciona os usuários com o perfil configurado
    $sql = 'SELECT u.id, u.firstname,u.lastname, u.email FROM {role_assignments} rs 
            INNER JOIN {user} u ON u.id = rs.userid
            INNER JOIN {context} e ON rs.contextid = e.id
            WHERE e.contextlevel = 50 AND rs.roleid IN (' . $pluginconfig->roles . ') AND e.instanceid = ' . $courseid->course;
    ...
    foreach ($users as $u) {
        $arr_return['selected'][$u->id] = $u->id;
        $arr_return['users'][$u->id] = $u->firstname . ' ' . $u->lastname;
        $i++;
    }

//    /$arr_return['selected'] = substr($arr_return['selected'], 0, -1);
